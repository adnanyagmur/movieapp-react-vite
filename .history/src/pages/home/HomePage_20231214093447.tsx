...
/*   const handleFavoriteClick = (movie: MovieItem) => {
  ...
  const handleFavoriteClick = (movie: MovieItem, isAlreadyFavorite: boolean) => {
    const favorites = JSON.parse(localStorage.getItem('favorites') || '[]');
    ...
    <Grid container spacing={1} sx={{m:3}}>
       ...
       {results.map((movie: MovieItem) => (
        ...
        <MovieCard
          ...
          onFavoriteClick={(isAlreadyFavorite: boolean) => handleFavoriteClick(movie, isAlreadyFavorite)}
        />
