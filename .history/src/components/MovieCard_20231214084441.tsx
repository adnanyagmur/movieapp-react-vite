...
console.log("detayyy", response)
    ...
    const handleToggleFavorite = () => {
      ...
      if (!isFavorite) {
        ...
        setFavoriteMovies((prevFavorites) => {
          const updatedFavorites = [...prevFavorites, favoriteData];
        ...
        setFavoriteMovies((prevFavorites) => {
          const updatedFavorites = prevFavorites.filter((movie) => movie.movieId !== movieId);
          localStorage.setItem('favoriteMovies', JSON.stringify(updatedFavorites));
          return updatedFavorites;
        });
      }
    ...
    // useEffect ile localStorage'ı takip et
    useEffect(() => {
      const storedFavorites: MovieCardProps[] = JSON.parse(localStorage.getItem('favoriteMovies') || '[]');
      setFavoriteMovies(storedFavorites);
    ...
    <Card sx={{ maxWidth: 250, mt: 3 }}>
      <CardActionArea>
        ...
        <CardContent sx={{ display: 'flex', flexDirection: 'column', flexGrow: 1 }}>
          ...
          <Rating name="movie-rating" size="small" value={voteAverage / 2} precision={0.5} readOnly sx={{ marginRight: '1rem' }} />
        </CardContent>
      </CardActionArea>
      <CardActions>
