...
import { Box, Button, Card, CardActionArea, CardContent, CardMedia, IconButton, Rating, Typography } from "@mui/material";
...
const titleLength = title.length
 ...
 const shortenedOverview = overview.length + titleLength > 100 ? `${overview.slice(0, 100-titleLength)}...` : overview;
  ...
  return (
    <Card sx={{ maxWidth: 250, maxHeight: '100%', mt: 3, display: 'flex', flexDirection: 'column' }}>
      <CardActionArea>
    <CardMedia
      component="img"
      alt={title}
    ...
    <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 'auto' }}>
        ...
        <Button variant="outlined" startIcon={<FavoriteIcon />} sx={{ fontSize: '0.8rem' }}>
          Favorite
        </Button>
        <Button variant="contained"  startIcon={<DetailsIcon />} sx={{ fontSize: '0.8rem' }}>
          Details
        </Button>
      </Box>
