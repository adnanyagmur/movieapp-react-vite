...
import { setMovieDetails } from "../redux/slice/movieSlice";
...
function MovieCard({ title, overview, releaseDate, posterPath, voteAverage , movieId, onFavoriteClick}: MovieCardProps) {
 ...
 //const [movieDetails, setMovieDetails] = useState(null);
    ...
    const fetchMovieDetails = async () => {
      const movieData = {
        title, overview, releaseDate, posterPath, voteAverage , movieId
      }
...
console.log("detayyy", response) */
 ...
 // console.log('Is Favorite:', isMovieInFavorites(movieId)); // Eklenecek log
  return (
        ...
        <Grid container justifyContent="space-between" alignItems="flex-end">
          <Grid item>
            <IconButton onClick={onFavoriteClick}>
              <FavoriteIcon sx={{color: isMovieInFavorites(movieId) ? "red": "disabled"}}  />
            </IconButton>
          </Grid>
          <Grid item>
