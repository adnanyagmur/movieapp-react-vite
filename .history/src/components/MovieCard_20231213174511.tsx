...
console.log("detayyy", response)
    ...
    // Favori filmleri tutacak state
    const [favoriteMovies, setFavoriteMovies] = useState <Array<MovieCardProps>>([]);
  
    useEffect(() => {
      // Sayfa yüklendiğinde localStorage'dan favori filmleri al
      const storedFavorites:Array<MovieCardProps>= JSON.parse(localStorage.getItem('favoriteMovies') || '[]');
      setFavoriteMovies(storedFavorites);

      // Favori filmler arasında bu film var mı kontrol et
    const isMovieFavorite = storedFavorites.some((movie:MovieCardProps) => movie.movieId === movieId);
