...
type  MovieCardProps ={
  ...
  posterPath: string;
  voteAverage: number;
  onFavoriteClick: (isAlreadyFavorite: boolean) => void;
  isFavorite: boolean;
}

function MovieCard({ title, overview, releaseDate, posterPath, voteAverage , movieId, onFavoriteClick,isFavorite}: MovieCardProps) {
...
console.log("detayyy", response)
 ...
 //   const isFavorite = JSON.parse(localStorage.getItem('favorites') || '[]').some((fav: any) => fav.movieId === movieId);
    ...
    console.log('Is Favorite:', isFavorite); // Eklenecek log
  return (
